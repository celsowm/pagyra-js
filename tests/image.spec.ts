...
import type { ImageRef, RenderBox } from "../src/pdf/types.js";

describe("Image handling tests", () => {
  const duckImagePath = join(process.cwd(), "tests/assets/images/duck.jpg");
  let duckImageBuffer: Buffer;
  let duckImageRef: ImageRef;
  ...
  function findImageNodes(node: RenderBox): RenderBox[] {
    const imageNodes: RenderBox[] = [];
