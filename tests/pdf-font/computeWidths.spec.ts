  ...
  it("computes DW as the mode and compresses W with ranges and lists", () => {
    ...
    for (const entry of W) {
      if (entry.length === 3) {
        ...
        const [start, end, value] = entry as [number, number, number];
        expect(value).not.toBe(DW);
      } else {
        // [start, [w1,w2,...]]
        const [start, arr] = entry as [number, number[]];
...
describe("buildToUnicodeCMap helper", () => {
  it("emits a minimal CMap with beginbfchar and entries", () => {
    // Import the helper function dynamically to avoid circular types; use the exported helper present in embedder.js
    // The helper buildToUnicodeCMap returns a string; we verify its structure
    // We'll import it via dynamic import (ESM)
  });
});
