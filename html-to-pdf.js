...
export function prepareHtmlRender(options) {
...
function offsetRenderTree(root, dx, dy, debug) {
    const stack = [root];
...
function convertDomNode(node, cssRules, parentStyle) {
...
function computeStyleForElement(element, cssRules, parentStyle) {
    const inherited = {
        color: parentStyle.color,
        fontSize: parentStyle.fontSize,
        lineHeight: parentStyle.lineHeight,
    ...
    if (styleInit.backgroundColor !== undefined)
        styleOptions.backgroundColor = styleInit.backgroundColor;
    if (styleInit.borderColor !== undefined)
        styleOptions.borderColor = styleInit.borderColor;
    if (styleInit.fontSize !== undefined)
        styleOptions.fontSize = styleInit.fontSize;
    ...
    if (styleInit.marginTop !== undefined)
        styleOptions.marginTop = styleInit.marginTop;
    if (styleInit.marginRight !== undefined)
        styleOptions.marginRight = styleInit.marginRight;
    if (styleInit.marginBottom !== undefined)
        styleOptions.marginBottom = styleInit.marginBottom;
    if (styleInit.marginLeft !== undefined)
        styleOptions.marginLeft = styleInit.marginLeft;
    if (styleInit.paddingTop !== undefined)
        styleOptions.paddingTop = styleInit.paddingTop;
    if (styleInit.paddingRight !== undefined)
        styleOptions.paddingRight = styleInit.paddingRight;
    if (styleInit.paddingBottom !== undefined)
        styleOptions.paddingBottom = styleInit.paddingBottom;
    if (styleInit.paddingLeft !== undefined)
        styleOptions.paddingLeft = styleInit.paddingLeft;
    if (styleInit.borderTop !== undefined)
        styleOptions.borderTop = styleInit.borderTop;
    if (styleInit.borderRight !== undefined)
        styleOptions.borderRight = styleInit.borderRight;
    if (styleInit.borderBottom !== undefined)
        styleOptions.borderBottom = styleInit.borderBottom;
    if (styleInit.borderLeft !== undefined)
        styleOptions.borderLeft = styleInit.borderLeft;
    if (styleInit.width !== undefined)
        styleOptions.width = styleInit.width;
    if (styleInit.height !== undefined)
        styleOptions.height = styleInit.height;
...
function applyDeclarationsToStyle(declarations, target) {
...
function applyBoxShorthand(value, apply) {
    const parts = value.split(/\s+/).filter(Boolean);
    if (parts.length === 0) {
        return;
    }
    const resolved = parts.map(parseLength);
    const [top, right, bottom, left] = resolved.length === 1
