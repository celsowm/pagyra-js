...
export class FontEmbedder {
    ...
    faceMetrics = new Map();
    constructor(config, doc // PdfDocument type
    ) {
        this.config = config;
        this.doc = doc;
    }
...
`;
        ...
        for (const [codePoint, glyphId] of metrics.cmap["unicodeMap"]) {
            if (totalMappings >= 100)
                break; // Limit for brevity
...
`;
        ...
        return this.doc.registerStream(new TextEncoder().encode(fullCmap), { Length: fullCmap.length });
    }
    loadFontData(path) {
