...
export function parseSelector(selector: string): Part[] | null {
  ...
  function parseSimpleToken(tok: string): Simple | null {
    ...
    while (rest.length) {
      ...
      if (ch === '[') {
        ...
        attrs.push(op === 'exists' ? { name, op } : { name, op, value: val! });
        rest = rest.slice(m[0].length);
        continue;
      }
      if (ch === ':') {
        ...
        if (m) {
          const nb = parseNth(m[1]); if (nb) pseudos.push({ kind: 'nth-child', a: nb.a, b: nb.b });
  ...
  for (let k = 0; k < tokens.length; k++) {
    ...
    if (typeof t !== 'string') {
      ...
      parts.push({ simple: t, combinatorToLeft: comb });
    }
  }
