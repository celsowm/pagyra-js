...
class CmapParser implements CmapData {
  public readonly unicodeMap = new Map<number, number>();

  constructor(cmapTable: DataView) {
    this.parseCmapTable(cmapTable);
  }

  private parseCmapTable(table: DataView): void {
  ...
  private parseFormat4Table(table: DataView, offset: number): void {
    const format = table.getUint16(offset, false);
    if (format !== 4) return; // only support format 4
...
export function parseTtfFont(filePath: string): TtfFontMetrics {
  ...
  try {
    ...
    for (let glyphId = 0; glyphId < numGlyphs; glyphId++) {
      ...
      if (offset + 4 > hmtxTable.byteLength) break;
